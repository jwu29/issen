...
\begin{enumerate}
    ...
    \item \textbf{Contribution 2 (Section~\ref{sec:properties}, Theorem~\ref{thm:parser-indep}).}
    A semantically strong Parser Independence theorem: identical bytes
    yield identical records under a fixed parser, regardless of the
    navigation primitive that retrieved the bytes. This elevates a folk
    design principle to a formal claim with empirical content
    ...
    \item \textbf{Contribution 4 (Section~\ref{sec:properties}, Theorem~\ref{thm:layer-rule}, and Section~\ref{sec:implementation}).}
    The PARSER-NAVIGATE layer dependency rule, derived as the contrapositive
    of Theorem~\ref{thm:parser-indep}; the rule converts a folk software
    discipline into a formal architectural constraint that build systems can
    enforce, and we instantiate it in Issen's crate boundaries.
...
Section~\ref{sec:properties} states and proves the central theorems.
