...
use rt_signatures::engines::stix::StixParser;
    ...
    if let Some(bundles) = stix_bundles {
        for bundle_path in bundles {
            ...
            for indicator in &indicators {
                use rt_signatures::engines::stix::ExtractedIoc;
...
fn print_text_report(file_path: &Path, findings: &[rt_signatures::matching::results::ScanFinding]) {
...
fn print_json_reports(
    reports: &[(
        std::path::PathBuf,
        Vec<rt_signatures::matching::results::ScanFinding>,
    )],
) -> Result<()> {
...
fn load_engine_from_feeds() -> Result<ScanEngine> {
    ...
    let registry = rt_signatures::feeds::config::FeedRegistry::with_defaults(&feed_cache_dir);
    let feed_cache = rt_signatures::feeds::fetcher::FeedCache::new(&feed_cache_dir);

    let (engine, summary) = rt_signatures::feeds::loader::load_cached_feeds(&registry, &feed_cache)
...
fn default_feed_cache_dir() -> std::path::PathBuf {
    if let Ok(home) = std::env::var("HOME") {
        std::path::Path::new(&home).join(".local/share/rapidtriage/feeds")
    } else {
