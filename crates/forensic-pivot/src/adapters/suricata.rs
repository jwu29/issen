...
mod tests {
    ...
    fn test_eve_aleissen_captures_src_and_dest_ip_in_attrs() {
        ...
        let ev: Evidence = alert.into();
        assert_eq!(
            ev.attrs.get("src_ip").map(std::string::String::as_str),
            Some("192.168.1.100")
        );
        assert_eq!(
            ev.attrs.get("dest_ip").map(std::string::String::as_str),
            Some("10.0.0.1")
        );
        assert_eq!(
            ev.attrs.get("src_port").map(std::string::String::as_str),
            Some("4444")
        );
        assert_eq!(
            ev.attrs.get("dest_port").map(std::string::String::as_str),
            Some("3333")
        );
    }
