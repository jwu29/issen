...
mod tests {
    ...
    impl CollectionProvider for AlwaysHighProvider {
        ...
        fn probe(&self, _path: &Path) -> Result<Confidence, rt_core::error::RtError> {
            Ok(Confidence::High)
        }

        fn open(&self, path: &Path) -> Result<CollectionManifest, rt_core::error::RtError> {
            let tempdir = tempfile::tempdir().map_err(rt_core::error::RtError::Io)?;
    ...
    impl CollectionProvider for AlwaysErrProvider {
        ...
        fn probe(&self, _path: &Path) -> Result<Confidence, rt_core::error::RtError> {
            Err(rt_core::error::RtError::InvalidData(
                "mock probe error".to_string(),
            ))
        }

        fn open(&self, _path: &Path) -> Result<CollectionManifest, rt_core::error::RtError> {
            Err(rt_core::error::RtError::UnsupportedFormat(
