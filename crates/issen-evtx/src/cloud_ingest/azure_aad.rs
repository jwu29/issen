...
mod tests {
    ...
    #[test]
    fn parse_empty_array_returns_empty() { assert!(parse_azure_aad_signin("[]").is_empty()); }
    #[test]
    fn parse_invalid_json_returns_empty() { assert!(parse_azure_aad_signin("not json").is_empty()); }
    #[test]
    ...
    fn parse_maps_user_principal_name_to_data() {
        let events = parse_azure_aad_signin(SAMPLE_SIGNIN);
        assert!(events[0].data.contains_key("userPrincipalName") || events[0].data.contains_key("TargetUserName"));
    }
    #[test]
