    ...
    records
        ...
        .filter_map(|rec| {
            ...
            if let Some(folder) = rec.get("FolderPath").and_then(|v| v.as_str()) {
                ...
                let image = format!(
                    "{folder}\\{}",
                    rec.get("FileName").and_then(|v| v.as_str()).unwrap_or("")
                );
            ...
            if let Some(rip) = rec.get("RemoteIP").and_then(|v| v.as_str()) {
                if !rip.is_empty() {
                    data.insert("RemoteIP".into(), rip.into());
                }
            }
            if let Some(rport) = rec.get("RemotePort").and_then(serde_json::Value::as_u64) {
                if rport > 0 {
                    data.insert("RemotePort".into(), rport.to_string());
                }
            }
...
fn parse_iso8601_ns(s: &str) -> Option<i64> {
    ...
    if let Ok(dt) = jiff::civil::DateTime::strptime("%Y-%m-%dT%H:%M:%SZ", s) {
...
mod tests {
    ...
    fn parse_carries_filename_in_data() {
        ...
        assert!(events[0]
            .data
            .values()
            .any(|v| v.to_lowercase().contains("powershell")));
    }
    #[test]
