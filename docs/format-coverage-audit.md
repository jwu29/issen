...
| **git-forensic** ✅ FIXED | ~~only loose objects; packfiles unimplemented~~ → **pack v2 reader shipped**: index-v2 lookup + OFS/REF delta resolution; `read_object` reads packed objects (loose→pack), `PackfileUnsupported` only for non-v2 index. Validated vs the **real `git` binary** (blob/commit/tree + both delta types match `git cat-file`, SHA1 verified). | (was: every post-`gc` repo → misleading not-found) | DONE |
