...
3. **Each parser tags its emitted events** with the right `ActivityCategory` — added incrementally as each parser is touched (the parser knows its output's category; the *vocabulary* is forensicnomicon's, re-exported as `issen_core::ActivityCategory`). A formal forensicnomicon source→category lookup table (1b) is deferred until ≥2 parsers need to share one.
   - ✅ **usnjrnl** → `FileSystemActivity` (RED `c37555a` / GREEN `20e9364`); also added the issen-core re-export + the missing fleet test-lint opt-out.
   - ✅ **prefetch** → `Execution` (RED `49b160c` / GREEN `ad246fe`; + test-lint opt-out).
   - ⬜ Remaining parsers (pattern set — `.with_activity_category(...)` in the event builder + RED/GREEN): amcache→`Execution`, mft→`FileSystemActivity`, linux {auth_log,bash_history}→`LoginActivity` / syslog→`SystemState` / cron→`ScheduledTask` / boot_log→`Persistence`?, evtx (per event-id), registry-routed (runkeys→`Persistence`, userassist/shimcache→`Execution`, sam/lsadump→`AccountActivity`, …), srum→`Execution`/`NetworkActivity`, lnk→`FileSystemActivity`, setupapi→`DeviceInstall`, biome→`UserActivity`.
   - **(Resolved)** the working-tree dirty files were **pure rustfmt drift**, not another session — committed as `57a1493` (`style:` no-logic). Untracked `issen-disk/examples/dump_file.rs` left as-is (one-off, would add clippy debt).
